            ...
            <div className={`${isDark ? 'bg-[#111116] border-gray-800' : 'bg-white border-gray-300 shadow-2xl'} border rounded-2xl p-8 max-w-lg w-full text-center relative`}>
                ...
                <div className={`${isDark ? 'bg-[#0a0a0c] border-gray-800 text-gray-300' : 'bg-gray-100 border-gray-300 text-gray-800'} border rounded-xl p-5 text-left space-y-3 mb-6`}>
                    ...
                    <div className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'} leading-relaxed space-y-1.5 font-pixel`}>
                        ...
                        <p>• <strong className={isDark ? 'text-gray-300' : 'text-gray-700'}>核心：</strong>AI 模型驱动</p>
                        <p>• <strong className={isDark ? 'text-gray-300' : 'text-gray-700'}>功能：</strong>智能音乐推荐 · 多平台歌单同步 · 天气感知</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <button
                        onClick={onDismiss}
                        className={`border ${isDark ? 'border-gray-600 text-gray-400 hover:bg-gray-800 hover:text-white' : 'border-gray-300 text-gray-500 hover:bg-gray-100 hover:text-gray-700'} px-6 py-2.5 rounded-full transition-all font-pixel tracking-widest text-xs uppercase`}
                    >
                        今日不再展示
                    </button>
                    <button
